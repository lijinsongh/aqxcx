	...
	<view>
		...
		<uni-popup ref="popus" type="bottom">
			<view style="background: #FFFFFF;height: max-content; padding: 20upx;" v-if="payList.length > 0">
				<view style="display: flex;height: max-content;margin: 10upx 0 0 30upx;align-items: center;padding-bottom: 20upx;"
				 v-for="(item,index) in payList" :key='index'>
					<view style="display: flex;width: 100%;" v-if="lingqian != 0 && item.name == '钱包零钱'">
					...
					<view style="display: flex;width: 100%;" v-if="item.name != '钱包零钱'">
						<image :src="item.image" style="width: 50upx;height: 50upx;border-radius: 50upx;"></image>
	...
	export default {
		...
		methods: {
			lingJuan(item) {
				...
				if (parseFloat(myMoney) >= parseFloat(minMoney)) {
			...
			goPayment(ids, payNum) {
				...
				if (this.openWay === 0) {
					...
					// #ifdef H5
					this.$Request.getT('/aliPay/payH5?payNum=' + payNum + '&userId=' + userId + '&payMoney=' + this.MaxMoney).then(res => {
					...
					// #endif
					// #ifdef APP-PLUS
					this.$Request.getT('/aliPay/payApp?payNum=' + payNum + '&userId=' + userId + '&payMoney=' + this.MaxMoney).then(
						res => {
			...
			goPay() {
				...
				for (var i = 0; i < this.shopCartOrderList.length; i++) {
					let data = {
						userId: userId,
						status: this.shopCartOrderList[i].goods.status == '2' ? '9' : '1',
						orderType: this.shopCartOrderList[i].goods.status,
						consignee: this.consignee,
						mobile: this.mobile,
						provinces: this.provinces,
						detail: this.detail,
						//上面的都是地址信息
						storeId: this.shopCartOrderList[i].storeId,
						storeName: this.shopCartOrderList[i].storeName,
						merchantId: this.shopCartOrderList[i].merchantId,
						...
						payMoney: ((parseFloat(this.shopCartOrderList[i].goods.memberPrice) + parseFloat(this.shopCartOrderList[i].goods.postagePrice))).toFixed(2),
						relationId: '',
						detailJson: this.shopCartOrderList[i].sku ? this.shopCartOrderList[i].sku.detailJson : '',
						commissionPrice: ((parseFloat(this.shopCartOrderList[i].goods.memberPrice) * parseFloat(this.shopCartOrderList[i]
							.goods.commissionPrice)) * parseFloat(grade)).toFixed(2)
					}
					datas.push(data);
				}
