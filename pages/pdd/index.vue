	...
	export default {
		...
		methods: {
			loadPddCouponList: function(type) {
				...
				this.loadingType = 1;
				this.$Request.getT('/pdd/query/sortType/' + this.tabFromIndex + '/page/' + this.page + '/pageSize/10')
					.then(res => {
						this.loadingType = 0;
						if (res.topGoodsListGetResponse) {
							...
							res.topGoodsListGetResponse.list.forEach(d => {
								...
								d.tkmoneys = ((((d.minGroupPrice - d.couponDiscount) * 0.01) * (d.promotionRate * 0.001)) * parseFloat(this.$queue.maxMoney())).toFixed(2);
								if (grade) {
									d.tkmoney = ((((d.minGroupPrice - d.couponDiscount) * 0.01) * (d.promotionRate * 0.001)) * parseFloat(
										grade)).toFixed(2);
								} else {
									d.tkmoney = ((((d.minGroupPrice - d.couponDiscount) * 0.01) * (d.promotionRate * 0.001)) * parseFloat(this.$queue.minMoney())).toFixed(2);
								}
