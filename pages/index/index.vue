<template>
	<view class="index-content" style="background: white">
		<!-- banner板块 -->
		<view v-if="TabCur == 0">
			<view class="home-bg" style="height: 90upx;"></view>
			<view class="swiper" style="margin-top: -110upx;">
				<swiper class="swiper-container" :autoplay="true" :interval="4000" :circular="true"
					...
					<block v-for="(item, index3) in banners" :key="index3">
						<swiper-item class="swiper-wrapper" @tap="toGoodsInfo(item.url)">
							<image :src="item.image_url" style="width: 100%;height: 220rpx;border-radius: 32rpx;"
			...
			<view class="category" v-if="navlist.length>0">
				<swiper class="swiper" duration="300" :style="{ height: categoryHeight }" @change="switchMenu">
					<swiper-item v-for="(nav, index5) in navlist" :key="index5">
						<view class="category-list">
							<view class="icon" v-for="(item,ind) in nav" :key="ind"
								...
								<image mode="widthFix" :src="item.image_url" style="height: 90upx;width: 90upx"></image>
			...
			<!-- 领券 -->
			<view class="index-coupon has-pd-10" style="background: ghostwhite;"
				v-if="category[0].orderList.length > 0">
				...
				<view class="goods-list" v-if="category[0].orderList.length > 0">
					<orange-goods-card-home v-for="(item,index8) in category[0].orderList" :index="index8"
						...
						:itemtitle="item.skuName" :itemprice="'¥' + item.itemprice" :itemsale="item.itemsale"
						:itemendprice="'' + item.itemendprice" :couponmoney="item.couponmoney"></orange-goods-card-home>
				</view>
			</view>
		</view>
		<view class="index-coupon has-pd-10" style="margin:180upx 8upx 90upx 8upx ;background: #FFFFFF"
			v-if="TabCur != 0">
			<view class="category" v-if="category[TabCur].banner.length > 0">
				<view class="list">
					<view class="box" v-for="(item, i) in category[TabCur].banner" :key="item.son_name"
						@tap="toCategory(item.son_name)">
			...
			<view class="goods-list" v-if="category[TabCur].orderList.length > 0">
				...
				<view class="index-coupon has-bg-white has-pd-10">
					<view class="goods-list" v-if="category[TabCur].orderList.length > 0">
						<orange-goods-card-home v-for="(item,index) in category[TabCur].orderList" :index="index"
							:tkmoney="item.tkmoney" :tkmoneys="item.tkmoneys" :itemid="item.skuId" :logo="jdlogo"
							:isEnable="isEnable" :is-invitation="isInvitation" :itempic="item.itempic"
							:itemtitle="item.skuName" :itemprice="'¥' + item.itemprice" :itemsale="item.itemsale"
							:itemendprice="'' + item.itemendprice" :couponmoney="item.couponmoney">
	...
	export default {
		...
		data() {
			return {
				...
				navlists: [{
						...
						url: '/pages/index/list?title=淇湘优购优选&type=9',
					},
					{
						title: '拼多多',
				...
				categoryHeight: '300rpx', //菜单默认高度
				currentPageindex: 0, //菜单滚动小点
			};
		},
		onPullDownRefresh() {
			this.category[this.TabCur].page = 0;
			this.loadBanner();
			this.loadMenuList();
			this.loadMenuList1();
			this.loadMessage();
		...
		methods: {
			...
			loadMenuList: function() {
				this.$Request.getT('/activity/state/1?type=1').then(res => {
					if (res.status === 0) {
						if (res.data.length > 0) {
			...
			loadCouponList: function(type) {
				...
				this.category[this.TabCur].page = this.category[this.TabCur].page + 1;
				this.category[this.TabCur].loadingType = 1;
				if (this.TabCur == 0) {
					this.$Request.getT('/jd/goods/list/' + this.category[this.TabCur].page +
						'?sortName=inOrderCount30Days&keywords=').then(res => {
						...
						if (res.code === 200) {
							res.data.list.forEach(d => {
								...
								if (Math.round(d.priceInfo.price - d.couponInfo.couponList[0]
										.discount) > 0) {
								...
								if (grade) {
									d.tkmoney = (d.commissionInfo.commission * parseFloat(grade))
										.toFixed(2);
								} else {
									d.tkmoney = (d.commissionInfo.commission * 0.3).toFixed(2);
								}
								d.itemsale = d.inOrderCount30Days > 10000 ? '已售 ' + (d
										.inOrderCount30Days / 10000).toFixed(1) + '万' : '已售 ' +
					...
					this.$Request.getT('/jd/goods/list/' + this.category[this.TabCur].page +
						'?sortName=inOrderCount30Days&keywords=' +
						this.category[this.TabCur].search).then(res => {
						this.category[this.TabCur].loadingType = 0
						if (res.code === 200) {
			...
			tabSelect(e) {
				...
				// this.category[this.TabCur].page = 0;
				// this.category[e.currentTarget.dataset.id].orderList = [];
			...
			getBaiyi(index) {
				...
				this.$Request.getT("/pdd/getGeneratePromUrl?pid=" + pddpid).then(res => {
					if (res.resourceUrlResponse) {
						let weAppInfo = res.resourceUrlResponse.weAppInfo
						console.error(weAppInfo)
						if (weAppInfo) {
			...
			toNavList: function(url, title) {
				if (title === '话费充值') {
					...
					if (url.indexOf('/pages/index/eleme') !== -1) {
						uni.navigateTo({
							url: '/package/pages/index/eleme'
						});
					} else if (url.indexOf('/pages/index/meituan') !== -1) {
						uni.navigateTo({
							url: '/package/pages/index/meituan'
						});
					} else if (url.indexOf('/pages/index/duomai') !== -1) {
...
<style lang="scss">
	...
	.selectTop {
		...
		background: -ms-linear-gradient(left, #FF0000 0, #FF0000 100%);
		background: -webkit-gradient(linear, right top, left top, color-stop(0, #FF0000), to(#FF0000));
		background: -o-linear-gradient(right, #FF0000 0, #FF0000 100%);
