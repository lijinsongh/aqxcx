<script>
	...
	export default {
		...
		onLaunch: function() {
			...
			//是否开启免单商品分享
			that.$Request.getT('/common/type/241').then(res => {
				if (res.status == 0) {
					if (res.data && res.data.value) {
						this.$queue.setData('fenxiangMianDan', res.data.value);
					}
				}
			});
			//微信提现方式
			that.$Request.getT('/common/type/244').then(res => {
				if (res.status == 0) {
					if (res.data && res.data.value) {
						this.$queue.setData('wxtxType', res.data.value);
					}
				}
			});
			//评论关键字
			that.$Request.getT('/common/type/289').then(res => {
				if (res.status == 0) {
					if (res.data && res.data.value) {
						this.$queue.setData('pinlunGJZ', res.data.value);
					}
				}
			});
		}
...
<style lang="scss">
	/*每个页面公共css */
	@import './package/colorui/main.css';
	@import './package/colorui/icon.css';
